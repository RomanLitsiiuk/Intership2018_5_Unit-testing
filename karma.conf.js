...
module.exports = function (config) {
  config.set({
    ...
    files: [
      ...
      /* parameters:
               watched: if autoWatch is true all files that have set watched to true will be
                watched for changes
                served: should the files be served by Karma's webserver?
                included: should the files be included in the browser using <script> tag?
                nocache: should the files be served from disk on each request by Karma's
                 webserver? */
      /* assets:
    ...
    // config.LOG_DISABLE, config.LOG_ERROR, config.LOG_INFO, config.LOG_DEBUG
    ...
    reporters: ['kjhtml', 'progress', 'coverage-istanbul'
      /*'kjhtml', 'progress','mocha', 'dots','progress','spec', 'mocha'*/],
    ...
    client: {
      ...
      // if true, Karma clears the context window upon the completion of running the tests, true
      // is default
      clearContext: false,
      // run the tests on the same window as the client, without using iframe or a new window,
      // false is default
      runInParent: false,
      // true: runs the tests inside an iFrame; false: runs the tests in a new window,
      // true is default
      useIframe: true,
      jasmine: {
    ...
    /* karma-webpack config
       ...
       add `babel-loader` to the webpack configuration
        to make the ES6+ code readable to the browser */
    webpack: {
      module: {
        rules: [
          {
